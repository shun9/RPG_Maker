...
void ShunLib::Texture::Draw(const Matrix& world, const Matrix& view, const Matrix& proj, const Vec3& color, float alpha)
{
	...
	//各種行列を設定
	m_alphaTestEffect->SetWorld        (ShunLib::ConvertTK(world));
	m_alphaTestEffect->SetView         (ShunLib::ConvertTK(view));
	m_alphaTestEffect->SetProjection   (ShunLib::ConvertTK(proj));
